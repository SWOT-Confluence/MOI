...
from moi.ErrorStats import ErrorStats
...
class FlowLawCalibration:
    ...
    def CalibrateReach(self,verbose=True,optmethod='L-BFGS-B',suppress_warnings=False,lossfun='linear'):
        # Scoped deliberately.  A bare filterwarnings("ignore") here used to
        # persist for the rest of the process, so every warning raised after
        # the first calibration -- CORRIDORS resources being skipped among
        # them -- was silently swallowed.
        with warnings.catch_warnings():
            if suppress_warnings:
                warnings.simplefilter("ignore")
            return self._CalibrateReach(
                verbose=verbose, optmethod=optmethod, lossfun=lossfun
            )

    def _CalibrateReach(self,verbose=True,optmethod='L-BFGS-B',lossfun='linear'):
...
'''
    plotting functions commented out for now
